...
4.  **Protocolo LDAP (Lightweight Directory Access Protocol):** O idioma técnico que os sistemas usam para "consultar" o Active Directory. Quando você entra com login e senha no computador da empresa, o Windows faz uma consulta LDAP ao servidor AD: *"Esse usuário existe? A senha está correta? Quais grupos ele pertence?"* — tudo em milissegundos, invisível ao usuário final.
